...
class tiling_method_rectxy_scan_constz_around_a_point(tk.Frame):
    ...
    def get_tiling_coordinates_mm_and_cropFactors_rel(self) -> tuple[MeaCoor_mm,float,float]|None:
        """
        ...
        map_coor = self._generate_tiling_coordinates()
        
        if map_coor is None or len(map_coor.mapping_coordinates) == 0:
            ...
            return (map_coor,cropx_reduction,cropy_reduction)
    ...
    def _generate_tiling_coordinates(self) -> MeaCoor_mm|None:
        """
        Generates the mapping coordinates. Also stores them in its own variable and the main
        operation variable.
        
        Returns:
            MeaCoor_mm|None: The mapping coordinates (list of (x,y,z) tuples) or None if an error occurs
        """
        ...
        ret_MeaCoor_mm = self._calculate_tiling_coordinates(list_mapCoor_mm[0], cropx_reduction, cropy_reduction, image, cal)
        if len(ret_MeaCoor_mm.mapping_coordinates) == 0:
            messagebox.showerror('Error','No valid mapping coordinates generated. Please check the input parameters.')
            return None
        
        return ret_MeaCoor_mm

    def _calculate_tiling_coordinates(self, mapCoor_mm: MeaCoor_mm, cropx_reduction: float,
        ...
        Args:
            mapCoor_mm (MeaCoor_mm): The mapping coordinates to tile.

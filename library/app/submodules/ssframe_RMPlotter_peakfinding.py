...
from library.data_related.RamanMeasurement import RamanMeasurement, RamanMeasurement_Plotter, RamanMeasurement_Handler
...
class Frm_RamanMeasurement_Plotter(tk.Frame):
    ...
    # >>> Storage setup <<<
        self._RamanMeasurement:RamanMeasurement|None = None
    ...
    def _save_txt(self):
        ...
        """
        if not isinstance(self._RamanMeasurement,RamanMeasurement): messagebox.showerror('Error','No Raman measurement to save.'); return
        
        savepath = filedialog.asksaveasfilename(defaultextension='.txt',filetypes=[('Text file','*.txt')])
        
        handler = RamanMeasurement_Handler()
    ...
    def plot_spectra(self,mea:RamanMeasurement|None=None,mea_id:str|None=None) -> tuple[list,list]:
        """

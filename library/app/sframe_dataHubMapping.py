...
from library.data_related.MappingMeasurement import MappingMeasurement_Hub, MappingMeasurement_Unit, MappingMeasurement_Handler

from library.data_related import SaveParamsEnum

class Frm_DataHub_Mapping(tk.Frame):
    def __init__(self, master, getter_MappingHub:Callable[[],MappingMeasurement_Hub]|None=None,width_rel:float=1, height_rel:float=1,autosave:bool=False):
        """
        ...
        self._MappingHub = MappingMeasurement_Hub()
    ...
    def _autosave_dataHub(self, suppress_errors:bool=False) -> threading.Thread:
        ...
        while self._flg_isrunning.is_set():
            try:
                ...
                MappingMeasurement_Handler().save_MappingHub_database(
    ...
    def get_selected_MappingUnit(self) -> list[MappingMeasurement_Unit]:
        """
    ...
    def get_MappingHub(self) -> MappingMeasurement_Hub:
    ...
    def append_MappingUnit(self, unit: MappingMeasurement_Unit, persist:bool=True):
        """
    ...
    def load_dataHub(self) -> threading.Thread:
        try:
            ...
            # q_load = queue.Queue()
            MappingMeasurement_Handler().load_choose(mappingHub=self._MappingHub)
    ...
    def save_dataHub(self,callback=None) -> threading.Thread:
        ...
        try:
            ...
            thread = MappingMeasurement_Handler().save_MappingMeasurementHub_choose(self._MappingHub)
            thread.join()
    ...
    def save_selected_mappingUnit_ext(self,callback:Callable=None):
        ...
        try:
            ...
            thread = MappingMeasurement_Handler().save_MappingUnit_ext_prompt(unit,flg_saveraw=flg_saveraw)
            thread.join()
    ...
    def set_MappingHub(self, mappingHub:MappingMeasurement_Hub):
        """
    ...
    def append_RamanMeasurement_multi(self, measurement:RamanMeasurement, coor:tuple=(0,0,0)):
        """
        ...
        unit = MappingMeasurement_Unit(unit_name=unit_name)
...
class Frm_DataHub_Mapping_Plus(tk.Frame):
    ...
    def __init__(self, master,dataHub:Frm_DataHub_Mapping,width_rel:float=1,height_rel:float=1):
        ...
        self._mappingUnit:MappingMeasurement_Unit = None
    ...
    def set_mappingHub(self, mappingHub:MappingMeasurement_Hub):
        ...
        """
        assert isinstance(mappingHub, MappingMeasurement_Hub), "mappingHub must be a MappingMeasurement_Hub object"
        self._dataHub.set_MappingHub(mappingHub)
        self.update_tree_unit()
        
    def get_selected_MappingUnit(self) -> MappingMeasurement_Unit:
        """
    ...
    def get_selected_RamanMeasurement(self) -> list[RamanMeasurement]:
        """
...
def test_dataHub():
    from library.data_related.MappingMeasurement import generate_dummy_mappingHub

...
class SpectrometerController_Dummy(Class_SpectrometerController):
    def __init__(self, **kwargs) -> None:
        ...
        # For the simulation (dummy datasets)
        self.dataset = pd.read_csv(r'./example dataset/one dataset only/spectra_dummy.csv',header=0)
...
# Spectrometer acquisition control
    ...
    def measure_spectrum(self) -> tuple[pd.DataFrame,int,int]:
        ...
        # wavelength = self.dataset['Wavelength [nm]'].values
        # intensity = self.dataset['Intensity [a.u.]'].values

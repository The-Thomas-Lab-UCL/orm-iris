...
class MappingCoordinates_mm:
    ...
    def load_pickle(self, filename:str):
        ...
        with open(filename, 'rb') as f:
            data:MappingCoordinates_mm = pickle.load(f)
            if not isinstance(data, MappingCoordinates_mm):
...
class MappingCoordinatesHub(list[MappingCoordinates_mm]):
    ...
    def append(self, mapCoor:MappingCoordinates_mm):
        """
        ...
        """
        if not isinstance(mapCoor, MappingCoordinates_mm): raise TypeError(f"Expected MappingCoordinates, got {type(mapCoor)}")
    ...
    def extend(self, mapCoor:list[MappingCoordinates_mm]):
        """
        ...
        """
        if not isinstance(mapCoor, list) or not all(isinstance(coor, MappingCoordinates_mm) for coor in mapCoor):
            raise TypeError(f"Expected list of MappingCoordinates, got {type(mapCoor)}")
        super().extend(mapCoor)
        self._notify_observers()
        
    def pop(self, idx:int) -> MappingCoordinates_mm:
        """
    ...
    def get_list_MappingCoordinates(self,list_unitNames:list[str]) -> list[MappingCoordinates_mm]:
        """

...
from library.controllers import XYController, ZController, CameraController
...
class stage_measurement_hub(mp.Process):
    def __init__(self, xy_controller: XYController, z_controller: ZController,
        ...
        def _assign_and_send_coordinates(self):
            ...
            while self._flg_selfrunning.is_set():
                ...
                try:
                    ...
                    if req_type == stage_measurement_hub.Enum_CoorType.CLOSEST:
                        coor = self.get_closest_coordinate(timestamp)
                    elif req_type == stage_measurement_hub.Enum_CoorType.INTERPOLATE:
    ...
    class _child_CamProc():
        ...
        def _handle_image_requests(self):
            ...
            while self._flg_selfrunning.is_set():
                ...
                try:
                    ...
                    if request == stage_measurement_hub.Enum_CamCorrectionType.RAW:
                        proc_img = arr_img
                    elif request == stage_measurement_hub.Enum_CamCorrectionType.FLATFIELD:
                        proc_img = self._correct_arr_flatfield(arr_img)
                    elif request == stage_measurement_hub.Enum_CamCorrectionType.BACKGROUND_SUBTRACTION:
                        proc_img = self._correct_backgroundSubtraction_3channel(arr_img)
                    else:
                        return_pkg = self._handle_other_requests(request)
                        
                    if request in stage_measurement_hub.Enum_CamCorrectionType.__members__.values():
        ...
        def _handle_other_requests(self,request:tuple[Any]) -> Any:
            ...
            return_pkg = None
            if request[0] == stage_measurement_hub.Enum_CommandType.SET_FLATFIELD_GAIN:
                if not isinstance(request[1],float): raise ValueError('Invalid request type, gain is not a float')
                self._ff_gain = request[1]
            
            elif request[0] == stage_measurement_hub.Enum_CommandType.GET_FLATFIELD_GAIN:
                return_pkg = self._ff_gain
            
            elif request[0] == stage_measurement_hub.Enum_CommandType.FLATFIELD_REF:
            ...
            elif request[0] == stage_measurement_hub.Enum_CommandType.SAVE_FLATFIELD_REF:
            ...
            elif request[0] == stage_measurement_hub.Enum_CommandType.LOAD_FLATFIELD_REF:
                if not isinstance(request[1],str): raise ValueError('Invalid request type, reference image is not a string')
...
def initialise_proxy_stage(manager:mpm.SyncManager) -> tuple[XYController,ZController,CameraController,StageNamespace]:
    """
...
def generate_dummy_stageHub() -> tuple[stage_measurement_hub,XYController,ZController,CameraController,StageNamespace]:
    """
    ...
    stagehub = stage_measurement_hub(
        xy_controller=xyproxy,
        z_controller=zproxy,
        cam_controller=camproxy,
        namespace=stage_namespace
